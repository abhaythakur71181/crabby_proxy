...
- [x] **R3-H6** Removed the dead third accumulator: `track_bandwidth`/`incr_bandwidth`/`get_bandwidth` (Redis `cache:bw`) had no reader — deleted, plus the per-close async round trip. Source of truth is now tracker=enforcement, usage table=persistence. NOTE remaining: month-rollover for long-lived tunnels (re-check window in `add_and_over`) and forward (non-CONNECT h2) path doesn't persist usage rows — both flagged as follow-ups in code comments.
- [x] **R3-H5** Usage writer now batches via `recv_many` (up to 256) into one transaction (one fsync/batch — the SQLite throughput fix), retries row-by-row on batch failure. `submit` is async and applies up to 2s back-pressure (`send().await` timeout) before dropping, instead of dropping immediately on a full channel; capacity raised 4096→16384. Drops still increment `usage_records_dropped_total`.
...
- [x] **R3-M5/M6/M7** M5: event-bus subscriber wrapped in a reconnect loop (capped exponential backoff, shutdown-aware) — a Redis blip no longer permanently stops cross-instance invalidation; cross-instance config reload now routed through the M3-guarded `reload_config`. (Kept pub/sub; Redis Streams deferred.) M6: memory backend `get_pending`/`list_pending` implemented (ConnectionRequest already derives Clone) — approval workflow now works single-instance; stale "not supported" tests replaced. M7: Redis `count_connections`/`count_user_connections` now prune-and-count (verify backing conn key via EXISTS, SREM orphans) so crash/TTL-orphaned set members can't over-count and wrongly deny the per-user limit.
...
- [~] **R3-H13/M24** H13: API client defaults to **https** when the base URL has no scheme (was http → cleartext creds/token); nginx now sends a real **CSP**, **HSTS**, and `Referrer-Policy: strict-origin-when-cross-origin`. M24: removed the duplicate charting stack — deleted the unused shadcn `ui/chart.tsx` (only recharts importer) and dropped `recharts` from package.json + lock (pages use chart.js). Build verified green. REMAINING: H13 localStorage→HttpOnly cookie (needs backend session-cookie support); M23 error boundary + typed API + TS strict; M24 route-level code splitting (bundle still ~670 KB).
...
- [x] **R3-DOCS** Added `docs/ARCHITECTURE.md`, `docs/adr/` (6 ADRs for this round's security decisions), `docs/RUNBOOK.md`; fixed README test count (398→443+7), `--test-threads=1` note, and MIT license statement.
...
## STATUS: Round 3 — Phases 1-4 substantially COMPLETE. 18 commits on fix/audit-r3-criticals; 443 unit + 7 integration green; rustfmt-clean; clippy -D warnings enforced; ARCHITECTURE/ADR/RUNBOOK docs added.
Added since last status: M21, M22, L10(clippy), R3-DOCS.
